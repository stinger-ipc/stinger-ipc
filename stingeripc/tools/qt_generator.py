...
def main(
    ...
    """Generate Qt output for a Stinger interface."""
    ...
    # The templates live under the package `stingeripc/templates/qt` next to `tools`.
    # compute that from the tools directory: tools -> parent (stingeripc) -> templates/qt
    template_dir = (this_file.parent / ".." / "templates" / "qt").resolve()

    if not template_dir.is_dir():
        raise RuntimeError(f"Qt template directory not found: {template_dir}")
    ...
    cmake_template_dir = template_dir / "cmake"
