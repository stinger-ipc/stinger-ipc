    ...
    _broker(broker)
{
    ...
    topicStream << boost::format("simple/%1%/interface") % "+";
...
void SimpleDiscovery::SetDiscoveryCallback(const std::function<void(const std::string&)>& cb)
{
    std::lock_guard<std::mutex> lock(_mutex);
    _discovery_callback = cb;
}

boost::future<std::string> SimpleDiscovery::GetSingleton()
{
    std::lock_guard<std::mutex> lock(_mutex);
...
std::vector<std::string> SimpleDiscovery::GetInstanceIds() const
{
    std::lock_guard<std::mutex> lock(_mutex);
    return _instance_ids;
}

void SimpleDiscovery::_onMessage(const std::string& topic, const std::string& payload, const MqttProperties& mqttProps)
{
    // Check if this message is for our discovery topic
    std::stringstream topicPattern;
    topicPattern << boost::format("simple/%1%/interface") % "+";
