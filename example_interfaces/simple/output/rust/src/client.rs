...
impl<C: Mqtt5PubSub + Clone + Send + 'static> SimpleClient<C> {
    ...
    async fn start_trade_numbers(
        ...
        let topic_param_map = HashMap::from([
            ...
            ("method_name".to_string(), "trade_numbers".to_string()),
        ...
        let response_topic: String = strfmt(
            "client/{client_id}/Simple/method/+/response",
            &topic_param_map,
        )
        .unwrap();
                ...
                tokio::spawn(async move {
                    while let Some((value, opt_responder)) = rx_for_school_prop.recv().await {
                        ...
                        let topic_param_map = HashMap::from([
                            ...
                            (
                                "service_id".to_string(),
                                instance_id_for_school_prop.clone(),
                            ),
                            ("property_name".to_string(), "school".to_string()),
                            ("prefix".to_string(), client_id_for_school_prop.clone()),
                        ]);
