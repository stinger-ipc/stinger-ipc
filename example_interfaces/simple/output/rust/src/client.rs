...
impl<C: Mqtt5PubSub + Clone + Send + 'static> SimpleClient<C> {
    ...
    pub async fn new(mut connection: C, discovery_info: DiscoveredService) -> Self {
        ...
        let topic_any_property_update_response =
            format!("client/{}/Simple/propertyUpdateResponse", client_id);
    ...
    async fn start_trade_numbers(
        ...
        let response_topic: String = format!("client/{}/Simple/methodResponse", self.client_id);
        ...
        let props = self.properties.clone();
        {
            ...
            if let Some(mut rx_for_school_prop) = props.school.take_request_receiver() {
                tokio::spawn(async move {
                    while let Some((value, opt_responder)) = rx_for_school_prop.recv().await {
                        let payload_obj = SchoolProperty { name: value };
                        ...
                        if let Some(responder) = opt_responder {
                            let resp_topic = format!(
                                "client/{}/Simple/propertyUpdateResponse",
                            ...
                            {
                                let mut hashmap =
                                    resp_map_for_school_prop.lock().expect("Mutex was poisoned");
                            ...
                            match message::property_update_request(
                                ...
                                Ok(msg) => {
                                    let _publish_result =
                                        publisher_for_school_prop.publish(msg).await;
                                    let result = receiver.await;
                                    match result {
                                        Ok(MethodReturnCode::Success(opt_response_text)) => {
                                            if let Some(response_text) = opt_response_text {
                                                let resp_obj =
                                                    serde_json::from_str::<SchoolProperty>(
                                                        response_text.as_ref(),
                                                    )
                                                    .unwrap();

                                                let _ = responder.send(Some(resp_obj.name));
                                            } else {
                                                warn!("No response payload received for property update request for 'school'");
                                                let _ = responder.send(None);
                                            }
                                        }
                                        _ => {
                                            warn!(
                                                "Property update request for 'school' failed: {:?}",
                                                result
                                            );
                                            let _ = responder.send(None);
                                        }
                            ...
                            match message::property_update(
                                ...
                                Ok(msg) => {
                                    let _publish_result =
                                        publisher_for_school_prop.publish_nowait(msg);
                                }
                                Err(e) => {
                                    warn!("Failed to create property update message for 'school': {:?}", e);
                                    continue;
