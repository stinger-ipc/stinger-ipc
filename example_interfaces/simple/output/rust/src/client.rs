...
impl<C: Mqtt5PubSub + Clone + Send + 'static> SimpleClient<C> {
    ...
    pub async fn new(mut connection: C, discovery_info: DiscoveredService) -> Self {
        ...
        let topic_person_entered_signal = format!("simple/{}/signal/personEntered", discovery_info.interface_info.instance);
        let subscription_id_person_entered_signal = connection.subscribe(topic_person_entered_signal, QoS::ExactlyOnce, message_received_tx.clone()).await;
        let subscription_id_person_entered_signal = subscription_id_person_entered_signal.unwrap_or(u32::MAX);
        debug!("Subscription (id={}) to signal topic for 'person_entered'", subscription_id_person_entered_signal);
        

        // Subscribe to all the topics needed for properties.
        
        let topic_school_property_value = format!("simple/{}/property/school/value", discovery_info.interface_info.instance);
        let subscription_id_school_property_value = connection.subscribe(topic_school_property_value, QoS::AtLeastOnce, message_received_tx.clone()).await;
        let subscription_id_school_property_value = subscription_id_school_property_value.unwrap_or(u32::MAX);
        debug!("Subscription (id={}) to property value topic for 'school'", subscription_id_school_property_value);
    ...
    pub async fn trade_numbers(&mut self, your_number: i32)->Result<i32, MethodReturnCode> {
        ...
        info!("Received response for method 'trade_numbers': {:?}", return_code);
        match return_code {
            MethodReturnCode::Success(payload_str) => {
                
                let return_obj: TradeNumbersReturnValues = serde_json::from_str(payload_str.as_ref().map_or("{}", |v| v))
    ...
    pub async fn set_school(&mut self, value: String) -> MethodReturnCode {
        ...
        let data = SchoolProperty { 
            name: value
        };
        let topic: String = format!("simple/{}/property/school/setValue", self.service_instance_id);
        let correlation_id = Uuid::new_v4();
        let (sender, receiver) = oneshot::channel();
        {
            let mut hashmap = self.pending_responses.lock().expect("Mutex was poisoned");
        ...
        let msg = message::property_update_request_message(&topic, &data, self.properties.school_version.load(Ordering::Relaxed), correlation_id).unwrap();
                                ...
                                *guard = pl.name.clone();
                                ...
                                if let Some(version_str) = msg.user_properties.get("Version") {
                                    if let Ok(version_num) = version_str.parse::<u32>() {
                                        props.school_version.store(version_num, std::sync::atomic::Ordering::Relaxed);
                                    }
                                }
                                if opt_corr_id.is_some() {
                                let opt_sender = opt_corr_id
