...
A Rust library providing client, server, and payload functionality for the Simple IPC system: Example StingerAPI interface which demonstrates just a bit.
...
```bash
# Client example
cargo run --example simple_client_demo --features client

# Server example
cargo run --example simple_server_demo --features server
```
