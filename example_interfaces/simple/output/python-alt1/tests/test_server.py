...
class TestSimpleServerProperties:
    ...
    def test_school_property_publish(self, server, mock_connection, initial_property_values):
        ...
        published_list = mock_connection.find_published("<bound method Property.value_topic of <stingeripc.components.Property object at 0x79d9181336b0>>".format("+"))
        ...
        expected_topic = "<bound method Property.value_topic of <stingeripc.components.Property object at 0x79d9181336b0>>".format(server.instance_id)
    ...
    def test_school_receive(self, server, server_setup, mock_connection):
        ...
        incoming_msg = Message(
            topic="<bound method Property.update_topic of <stingeripc.components.Property object at 0x79d9181336b0>>".format(server.instance_id),
...
class TestSimpleServerSignals:
    ...
    def test_server_emit_person_entered(self, server, mock_connection):
        ...
        published_list = mock_connection.find_published("<bound method Signal.topic of <stingeripc.components.Signal object at 0x79d9199bdca0>>".format("+"))
        assert len(published_list) == 1, "No message was published for signal 'person_entered'"

        msg = published_list[0]
        expected_topic = "<bound method Signal.topic of <stingeripc.components.Signal object at 0x79d9199bdca0>>".format(server.instance_id)
        assert msg.topic == expected_topic, f"Published topic '{msg.topic}' does not match expected '{expected_topic}'"
