"""Discovery screen for finding TestableIPC servers."""
...
from testableipc.client import TestableClientDiscoverer, DiscoveredInstance, TestableClient
from testableipc.tui.helpers import get_package_version
import logging
...
class DiscoveryScreen(Screen):
    """Screen for discovering available TestableIPC servers."""
    ...
    def on_mount(self) -> None:
        ...
        self.discoverer = TestableClientDiscoverer(conn)
    ...
    def watch_incomplete_interfaces(self, old_interfaces: Set[str], new_interfaces: Set[str]) -> None:
    ...
    def _update_grid(self) -> None:
        ...
        if not self.discovered_instances and not self.incomplete_interfaces:
            grid.mount(Static("No instances discovered yet...", classes="no_servers"))
        else:
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if hasattr(event.button, "instance_id"):
            ...
            # Create the TestableClient
            conn = self.app.mqtt_connection
            client = TestableClient(conn, discovered_instance)

            # Store the client in the app for the Client screen to use
            self.app.testable_client = client
