...
struct TestAbleProperties {
    ...
    read_write_optional_struct_tx_channel: watch::Sender<Option<Option<AllTypes>>>,read_write_two_structs_topic: Arc<String>,
    ...
    read_write_optional_datetime_tx_channel: watch::Sender<Option<Option<chrono::DateTime<chrono::Utc>>>>,read_write_two_datetimes_topic: Arc<String>,
    ...
    read_write_optional_duration_tx_channel: watch::Sender<Option<Option<chrono::Duration>>>,read_write_two_durations_topic: Arc<String>,
    ...
    read_write_optional_binary_tx_channel: watch::Sender<Option<Option<Vec<u8>>>>,read_write_two_binaries_topic: Arc<String>,
...
impl TestAbleServer {
    ...
    pub async fn emit_single_optional_array_of_strings(&mut self, values: Option<Vec<i32>>) -> SentMessageFuture {
    ...
    /// It updates the local value, notifies any watchers, and publishes the new value.
    /// If there is an error, it can publish back if a response topic was provided.
    async fn update_read_write_optional_struct_value(publisher: MqttierClient, topic: Arc<String>, property_pointer: Arc<Mutex<Option<ReadWriteOptionalStructProperty>>>, watch_sender: watch::Sender<Option<Option<AllTypes>>>, msg: ReceivedMessage) -> SentMessageFuture
    {
        let payload_str = String::from_utf8_lossy(&msg.payload).to_string();
    ...
    /// It updates the local value, notifies any watchers, and publishes the new value.
    /// If there is an error, it can publish back if a response topic was provided.
    async fn update_read_write_optional_datetime_value(publisher: MqttierClient, topic: Arc<String>, property_pointer: Arc<Mutex<Option<ReadWriteOptionalDatetimeProperty>>>, watch_sender: watch::Sender<Option<Option<chrono::DateTime<chrono::Utc>>>>, msg: ReceivedMessage) -> SentMessageFuture
    {
        let payload_str = String::from_utf8_lossy(&msg.payload).to_string();
    ...
    /// It updates the local value, notifies any watchers, and publishes the new value.
    /// If there is an error, it can publish back if a response topic was provided.
    async fn update_read_write_optional_duration_value(publisher: MqttierClient, topic: Arc<String>, property_pointer: Arc<Mutex<Option<ReadWriteOptionalDurationProperty>>>, watch_sender: watch::Sender<Option<Option<chrono::Duration>>>, msg: ReceivedMessage) -> SentMessageFuture
    {
        let payload_str = String::from_utf8_lossy(&msg.payload).to_string();
    ...
    /// It updates the local value, notifies any watchers, and publishes the new value.
    /// If there is an error, it can publish back if a response topic was provided.
    async fn update_read_write_optional_binary_value(publisher: MqttierClient, topic: Arc<String>, property_pointer: Arc<Mutex<Option<ReadWriteOptionalBinaryProperty>>>, watch_sender: watch::Sender<Option<Option<Vec<u8>>>>, msg: ReceivedMessage) -> SentMessageFuture
    {
        let payload_str = String::from_utf8_lossy(&msg.payload).to_string();
