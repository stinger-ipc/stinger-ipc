...
impl FullServer {
    pub async fn new(connection: &mut MqttierClient, method_handlers: Arc<AsyncMutex<Box<dyn FullMethodHandlers>>> ) -> Self {
        ...
        let subscription_id_add_numbers_method_req = connection.subscribe("full/method/addNumbers".to_string(), 2, message_received_tx.clone()).await;
        let subscription_id_add_numbers_method_req = subscription_id_add_numbers_method_req.unwrap_or_else(|_| usize::MAX);
        let subscription_id_do_something_method_req = connection.subscribe("full/method/doSomething".to_string(), 2, message_received_tx.clone()).await;
        let subscription_id_do_something_method_req = subscription_id_do_something_method_req.unwrap_or_else(|_| usize::MAX);
        let subscription_id_echo_method_req = connection.subscribe("full/method/echo".to_string(), 2, message_received_tx.clone()).await;
        let subscription_id_echo_method_req = subscription_id_echo_method_req.unwrap_or_else(|_| usize::MAX);
        
        
        let subscription_id_favorite_number_property_update = connection.subscribe("full/property/favoriteNumber/setValue".to_string(), 2, message_received_tx.clone()).await;
        ...
        let subscription_id_favorite_foods_property_update = connection.subscribe("full/property/favoriteFoods/setValue".to_string(), 2, message_received_tx.clone()).await;
        ...
        let subscription_id_lunch_menu_property_update = connection.subscribe("full/property/lunchMenu/setValue".to_string(), 2, message_received_tx.clone()).await;
        let subscription_id_lunch_menu_property_update = subscription_id_lunch_menu_property_update.unwrap_or_else(|_| usize::MAX);
        
        let subscription_id_family_name_property_update = connection.subscribe("full/property/familyName/setValue".to_string(), 2, message_received_tx.clone()).await;
    ...
    pub async fn emit_today_is(&mut self, day_of_month: i32, day_of_week: Option<DayOfTheWeek>) -> SentMessageFuture {
        ...
        let published_oneshot = self.mqttier_client.publish_structure("full/signal/todayIs".to_string(), &data).await;
    ...
    pub async fn emit_bark(&mut self, word: String) -> SentMessageFuture {
        ...
        let published_oneshot = self.mqttier_client.publish_structure("full/signal/bark".to_string(), &data).await;
        FullServer::oneshot_to_future(published_oneshot).await
    }
    ...
    async fn update_favorite_number_value(publisher: MqttierClient, topic: Arc<String>, data: Arc<Mutex<Option<i32>>>, watch_sender: watch::Sender<Option<i32>>, msg: ReceivedMessage) -> SentMessageFuture
    {
        ...
        let _ = FullServer::publish_favorite_number_value(publisher, topic2, data2).await
    }
    ...
    pub async fn set_favorite_number(&mut self, data: i32) {
        ...
        let _ = self.properties.favorite_number_tx_channel.send(Some(data_to_send_to_watchers));
    ...
    async fn update_favorite_foods_value(publisher: MqttierClient, topic: Arc<String>, data: Arc<Mutex<Option<FavoriteFoodsProperty>>>, watch_sender: watch::Sender<Option<FavoriteFoodsProperty>>, msg: ReceivedMessage) -> SentMessageFuture
    {
        ...
        let _ = FullServer::publish_favorite_foods_value(publisher, topic2, data2).await
    }
    ...
    pub async fn set_favorite_foods(&mut self, data: FavoriteFoodsProperty) {
        ...
        let data_to_send_to_watchers = data.clone();
        let _ = self.properties.favorite_foods_tx_channel.send(Some(data_to_send_to_watchers));

        let publisher2 = self.mqttier_client.clone();
        let topic2 = self.properties.favorite_foods_topic.as_ref().clone();
        let _ = tokio::spawn(async move {
            println!("Will publish property favorite_foods of type FavoriteFoodsProperty to {}", topic2);
    ...
    async fn update_lunch_menu_value(publisher: MqttierClient, topic: Arc<String>, data: Arc<Mutex<Option<LunchMenuProperty>>>, watch_sender: watch::Sender<Option<LunchMenuProperty>>, msg: ReceivedMessage) -> SentMessageFuture
    {
        ...
        let _ = FullServer::publish_lunch_menu_value(publisher, topic2, data2).await
    }
    ...
    pub async fn set_lunch_menu(&mut self, data: LunchMenuProperty) {
        ...
        let data_to_send_to_watchers = data.clone();
        let _ = self.properties.lunch_menu_tx_channel.send(Some(data_to_send_to_watchers));

        let publisher2 = self.mqttier_client.clone();
        let topic2 = self.properties.lunch_menu_topic.as_ref().clone();
        let _ = tokio::spawn(async move {
            println!("Will publish property lunch_menu of type LunchMenuProperty to {}", topic2);
    ...
    async fn update_family_name_value(publisher: MqttierClient, topic: Arc<String>, data: Arc<Mutex<Option<String>>>, watch_sender: watch::Sender<Option<String>>, msg: ReceivedMessage) -> SentMessageFuture
    {
        ...
        let _ = FullServer::publish_family_name_value(publisher, topic2, data2).await
    }
    ...
    pub async fn set_family_name(&mut self, data: String) {
        ...
        let data_to_send_to_watchers = data.clone();
        let _ = self.properties.family_name_tx_channel.send(Some(data_to_send_to_watchers));

        let publisher2 = self.mqttier_client.clone();
        let topic2 = self.properties.family_name_topic.as_ref().clone();
        let _ = tokio::spawn(async move {
            println!("Will publish property family_name of type String to {}", topic2);
