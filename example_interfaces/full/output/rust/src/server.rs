...
struct FullProperties {
    ...
    breakfast_length: Arc<Mutex<Option<std::time::Duration>>>,
...
impl FullServer {
    pub async fn new(connection: &mut MqttierClient, method_handlers: Arc<AsyncMutex<Box<dyn FullMethodHandlers>>> , instance_id: String) -> Self {
        ...
        let subscription_id_favorite_number_property_update = connection.subscribe(format!("full/{}/property/favoriteNumber/setValue", instance_id), 2, message_received_tx.clone()).await;
        ...
        let subscription_id_favorite_foods_property_update = connection.subscribe(format!("full/{}/property/favoriteFoods/setValue", instance_id), 2, message_received_tx.clone()).await;
        ...
        let subscription_id_lunch_menu_property_update = connection.subscribe(format!("full/{}/property/lunchMenu/setValue", instance_id), 2, message_received_tx.clone()).await;
        let subscription_id_lunch_menu_property_update = subscription_id_lunch_menu_property_update.unwrap_or_else(|_| usize::MAX);
        
        let subscription_id_family_name_property_update = connection.subscribe(format!("full/{}/property/familyName/setValue", instance_id), 2, message_received_tx.clone()).await;
        let subscription_id_family_name_property_update = subscription_id_family_name_property_update.unwrap_or_else(|_| usize::MAX);
        
        let subscription_id_last_breakfast_time_property_update = connection.subscribe(format!("full/{}/property/lastBreakfastTime/setValue", instance_id), 2, message_received_tx.clone()).await;
        ...
        let subscription_id_breakfast_length_property_update = connection.subscribe(format!("full/{}/property/breakfastLength/setValue", instance_id), 2, message_received_tx.clone()).await;
        ...
        let subscription_id_last_birthdays_property_update = connection.subscribe(format!("full/{}/property/lastBirthdays/setValue", instance_id), 2, message_received_tx.clone()).await;
    ...
    pub async fn emit_today_is(&mut self, day_of_month: i32, day_of_week: Option<DayOfTheWeek>, timestamp: chrono::DateTime<chrono::Utc>, process_time: std::time::Duration, memory_segment: Vec<u8>) -> SentMessageFuture {
    ...
    async fn handle_how_off_is_the_clock_request(publisher: MqttierClient, handlers: Arc<AsyncMutex<Box<dyn FullMethodHandlers>>>, msg: ReceivedMessage) {
        ...
        // call the method handler
        let rc: Result<std::time::Duration, MethodReturnCode> = {
            let handler_guard = handlers.lock().await;
    ...
    async fn publish_breakfast_length_value(publisher: MqttierClient, topic: String, data: std::time::Duration) -> SentMessageFuture
    {
    ...
    /// It updates the local value, notifies any watchers, and publishes the new value.
    /// If there is an error, it can publish back if a response topic was provided.
    async fn update_breakfast_length_value(publisher: MqttierClient, topic: Arc<String>, data: Arc<Mutex<Option<std::time::Duration>>>, watch_sender: watch::Sender<Option<std::time::Duration>>, msg: ReceivedMessage) -> SentMessageFuture
    {
        let payload_str = String::from_utf8_lossy(&msg.payload).to_string();
    ...
    pub async fn watch_breakfast_length(&self) -> watch::Receiver<Option<std::time::Duration>> {
        self.properties.breakfast_length_tx_channel.subscribe()
    }

    
    pub async fn set_breakfast_length(&mut self, data: std::time::Duration) -> SentMessageFuture {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let info = crate::interface::InterfaceInfo::new()
                    ...
                    .build();
                let _ = interface_publisher.publish_status(format!("full/{}/interface", interface_publisher.client_id), &info, 150).await;
            }
        });
...
pub trait FullMethodHandlers: Send + Sync {
    ...
    async fn handle_forward_time(&self, adjustment: std::time::Duration) -> Result<chrono::DateTime<chrono::Utc>, MethodReturnCode>;
    
    /// Pointer to a function to handle the how_off_is_the_clock method request.
    async fn handle_how_off_is_the_clock(&self, actual_time: chrono::DateTime<chrono::Utc>) -> Result<std::time::Duration, MethodReturnCode>;
