...
pub struct FullProperties {
    ...
    pub breakfast_length: Arc<Mutex<Option<std::time::Duration>>>,
    
    breakfast_length_tx_channel: watch::Sender<Option<std::time::Duration>>,
...
impl FullClient {
    ...
    pub async fn new(connection: &mut MqttierClient, service_id: String) -> Self {
        ...
        let topic_add_numbers_method_resp = format!("client/{}/addNumbers/response", connection.client_id);
        ...
        let topic_do_something_method_resp = format!("client/{}/doSomething/response", connection.client_id);
        ...
        let subscription_id_echo_method_resp = connection.subscribe(topic_echo_method_resp, 2, message_received_tx.clone()).await;
        let subscription_id_echo_method_resp = subscription_id_echo_method_resp.unwrap_or_else(|_| usize::MAX);
        let topic_what_time_is_it_method_resp = format!("client/{}/what_time_is_it/response", connection.client_id);
        let subscription_id_what_time_is_it_method_resp = connection.subscribe(topic_what_time_is_it_method_resp, 2, message_received_tx.clone()).await;
        let subscription_id_what_time_is_it_method_resp = subscription_id_what_time_is_it_method_resp.unwrap_or_else(|_| usize::MAX);
        let topic_set_the_time_method_resp = format!("client/{}/set_the_time/response", connection.client_id);
        let subscription_id_set_the_time_method_resp = connection.subscribe(topic_set_the_time_method_resp, 2, message_received_tx.clone()).await;
        let subscription_id_set_the_time_method_resp = subscription_id_set_the_time_method_resp.unwrap_or_else(|_| usize::MAX);
        let topic_forward_time_method_resp = format!("client/{}/forward_time/response", connection.client_id);
        let subscription_id_forward_time_method_resp = connection.subscribe(topic_forward_time_method_resp, 2, message_received_tx.clone()).await;
        let subscription_id_forward_time_method_resp = subscription_id_forward_time_method_resp.unwrap_or_else(|_| usize::MAX);
        let topic_how_off_is_the_clock_method_resp = format!("client/{}/how_off_is_the_clock/response", connection.client_id);
        let subscription_id_how_off_is_the_clock_method_resp = connection.subscribe(topic_how_off_is_the_clock_method_resp, 2, message_received_tx.clone()).await;
        let subscription_id_how_off_is_the_clock_method_resp = subscription_id_how_off_is_the_clock_method_resp.unwrap_or_else(|_| usize::MAX);
        ...
        let topic_favorite_number_property_value = format!("full/{}/property/favoriteNumber/value", service_id);
        ...
        let topic_favorite_foods_property_value = format!("full/{}/property/favoriteFoods/value", service_id);
        ...
        let topic_lunch_menu_property_value = format!("full/{}/property/lunchMenu/value", service_id);
        ...
        let topic_family_name_property_value = format!("full/{}/property/familyName/value", service_id);
        let subscription_id_family_name_property_value = connection.subscribe(topic_family_name_property_value, 1, message_received_tx.clone()).await;
        let subscription_id_family_name_property_value = subscription_id_family_name_property_value.unwrap_or_else(|_| usize::MAX);
        
        let topic_last_breakfast_time_property_value = format!("full/{}/property/lastBreakfastTime/value", service_id);
        let subscription_id_last_breakfast_time_property_value = connection.subscribe(topic_last_breakfast_time_property_value, 1, message_received_tx.clone()).await;
        let subscription_id_last_breakfast_time_property_value = subscription_id_last_breakfast_time_property_value.unwrap_or_else(|_| usize::MAX);
        
        let topic_breakfast_length_property_value = format!("full/{}/property/breakfastLength/value", service_id);
        let subscription_id_breakfast_length_property_value = connection.subscribe(topic_breakfast_length_property_value, 1, message_received_tx.clone()).await;
        let subscription_id_breakfast_length_property_value = subscription_id_breakfast_length_property_value.unwrap_or_else(|_| usize::MAX);
        
        let topic_last_birthdays_property_value = format!("full/{}/property/lastBirthdays/value", service_id);
        let subscription_id_last_birthdays_property_value = connection.subscribe(topic_last_birthdays_property_value, 1, message_received_tx.clone()).await;
        let subscription_id_last_birthdays_property_value = subscription_id_last_birthdays_property_value.unwrap_or_else(|_| usize::MAX);
        

        
        let property_values = FullProperties {
            
            favorite_number: Arc::new(Mutex::new(None)),favorite_number_tx_channel: watch::channel(None).0,
            favorite_foods: Arc::new(Mutex::new(None)),favorite_foods_tx_channel: watch::channel(None).0,
            lunch_menu: Arc::new(Mutex::new(None)),lunch_menu_tx_channel: watch::channel(None).0,
            
            family_name: Arc::new(Mutex::new(None)),family_name_tx_channel: watch::channel(None).0,
            
            last_breakfast_time: Arc::new(Mutex::new(None)),last_breakfast_time_tx_channel: watch::channel(None).0,
            
            breakfast_length: Arc::new(Mutex::new(None)),breakfast_length_tx_channel: watch::channel(None).0,
            last_birthdays: Arc::new(Mutex::new(None)),last_birthdays_tx_channel: watch::channel(None).0,
        };
    ...
    pub async fn add_numbers(&mut self, first: i32, second: i32, third: Option<i32>)->Result<i32, MethodReturnCode> {
        ...
        let _ = self.mqttier_client.publish_request(format!("full/{}/method/addNumbers", self.service_instance_id), &data, response_topic, correlation_data).await;
    ...
    pub async fn do_something(&mut self, a_string: String)->Result<DoSomethingReturnValue, MethodReturnCode> {
        ...
        let _ = self.mqttier_client.publish_request(format!("full/{}/method/doSomething", self.service_instance_id), &data, response_topic, correlation_data).await;
        let resp_obj = receiver.await.unwrap();
    ...
    pub async fn echo(&mut self, message: String)->Result<String, MethodReturnCode> {
        ...
        let data = EchoRequestObject {
            message: message,
        };

        let response_topic: String = format!("client/{}/echo/response", self.client_id);
        let _ = self.mqttier_client.publish_request(format!("full/{}/method/echo", self.service_instance_id), &data, response_topic, correlation_data).await;
        let resp_obj = receiver.await.unwrap();
    ...
    /// It finds oneshot channel created for the method call, and sends the response to that channel.
    fn handle_echo_response(pending_responses: Arc<Mutex<HashMap::<Uuid, oneshot::Sender::<JsonValue>>>>, payload: String, opt_correlation_id: Option<Uuid>) {
    ...
    ///
    /// This method awaits on the response to the call before returning.
    pub async fn what_time_is_it(&mut self, the_first_time: chrono::DateTime<chrono::Utc>)->Result<chrono::DateTime<chrono::Utc>, MethodReturnCode> {
        let correlation_id = Uuid::new_v4();
        let correlation_data = correlation_id.as_bytes().to_vec();
        let (sender, receiver) = oneshot::channel();
        {
        ...
        let _ = self.mqttier_client.publish_request(format!("full/{}/method/whatTimeIsIt", self.service_instance_id), &data, response_topic, correlation_data).await;
        let resp_obj = receiver.await.unwrap();
        {
            let dt_str = resp_obj["timestamp"].as_str().unwrap();
            let dt = chrono::DateTime::parse_from_rfc3339(dt_str)
                .map_err(|e| MethodReturnCode::DeserializationError(format!("Failed to parse datetime: {}", e)))?
    ...
    /// It finds oneshot channel created for the method call, and sends the response to that channel.
    fn handle_what_time_is_it_response(pending_responses: Arc<Mutex<HashMap::<Uuid, oneshot::Sender::<JsonValue>>>>, payload: String, opt_correlation_id: Option<Uuid>) {
    ...
    pub async fn set_the_time(&mut self, the_first_time: chrono::DateTime<chrono::Utc>, the_second_time: chrono::DateTime<chrono::Utc>)->Result<SetTheTimeReturnValue, MethodReturnCode> {
        ...
        let _ = self.mqttier_client.publish_request(format!("full/{}/method/setTheTime", self.service_instance_id), &data, response_topic, correlation_data).await;
        let resp_obj = receiver.await.unwrap();
        Ok(SetTheTimeReturnValue {
            ...
            confirmation_message: resp_obj["confirmation_message"].as_str().unwrap().to_string(),
        })
    ...
    /// It finds oneshot channel created for the method call, and sends the response to that channel.
    fn handle_set_the_time_response(pending_responses: Arc<Mutex<HashMap::<Uuid, oneshot::Sender::<JsonValue>>>>, payload: String, opt_correlation_id: Option<Uuid>) {
    ...
    ///
    /// This method awaits on the response to the call before returning.
    pub async fn forward_time(&mut self, adjustment: std::time::Duration)->Result<chrono::DateTime<chrono::Utc>, MethodReturnCode> {
        let correlation_id = Uuid::new_v4();
        let correlation_data = correlation_id.as_bytes().to_vec();
        let (sender, receiver) = oneshot::channel();
        {
        ...
        let _ = self.mqttier_client.publish_request(format!("full/{}/method/forwardTime", self.service_instance_id), &data, response_topic, correlation_data).await;
        let resp_obj = receiver.await.unwrap();
        {
    ...
    /// It finds oneshot channel created for the method call, and sends the response to that channel.
    fn handle_forward_time_response(pending_responses: Arc<Mutex<HashMap::<Uuid, oneshot::Sender::<JsonValue>>>>, payload: String, opt_correlation_id: Option<Uuid>) {
    ...
    ///
    /// This method awaits on the response to the call before returning.
    pub async fn how_off_is_the_clock(&mut self, actual_time: chrono::DateTime<chrono::Utc>)->Result<std::time::Duration, MethodReturnCode> {
        let correlation_id = Uuid::new_v4();
        let correlation_data = correlation_id.as_bytes().to_vec();
        let (sender, receiver) = oneshot::channel();
        {
        ...
        let response_topic: String = format!("client/{}/how_off_is_the_clock/response", self.client_id);
        let _ = self.mqttier_client.publish_request(format!("full/{}/method/howOffIsTheClock", self.service_instance_id), &data, response_topic, correlation_data).await;
        let resp_obj = receiver.await.unwrap();
        {
    ...
    /// It finds oneshot channel created for the method call, and sends the response to that channel.
    fn handle_how_off_is_the_clock_response(pending_responses: Arc<Mutex<HashMap::<Uuid, oneshot::Sender::<JsonValue>>>>, payload: String, opt_correlation_id: Option<Uuid>) {
    ...
    pub fn set_favorite_number(&mut self, value: i32) -> Result<(), MethodReturnCode> {
        let data = FavoriteNumberProperty { 
            number: value
        };
        let _publish_result = self.mqttier_client.publish_structure("full/{}/property/favoriteNumber/setValue".to_string(), &data);
        Ok(())
    }
    ...
    pub fn set_favorite_foods(&mut self, value: FavoriteFoodsProperty) -> Result<(), MethodReturnCode> {
        let data = value;
        let _publish_result = self.mqttier_client.publish_structure("full/{}/property/favoriteFoods/setValue".to_string(), &data);
        Ok(())
    }
    ...
    pub fn set_lunch_menu(&mut self, value: LunchMenuProperty) -> Result<(), MethodReturnCode> {
        let data = value;
        let _publish_result = self.mqttier_client.publish_structure("full/{}/property/lunchMenu/setValue".to_string(), &data);
        Ok(())
    }
    ...
    pub fn set_family_name(&mut self, value: String) -> Result<(), MethodReturnCode> {
        let data = FamilyNameProperty { 
            family_name: value
        };
        let _publish_result = self.mqttier_client.publish_structure("full/{}/property/familyName/setValue".to_string(), &data);
        Ok(())
    }
    ...
    /// This returns a watch::Receiver that can be awaited on for changes to the property value.
    pub fn watch_last_breakfast_time(&self) -> watch::Receiver<Option<chrono::DateTime<chrono::Utc>>> {
        self.properties.last_breakfast_time_tx_channel.subscribe()
    }
    
    pub fn set_last_breakfast_time(&mut self, value: chrono::DateTime<chrono::Utc>) -> Result<(), MethodReturnCode> {
        let data = LastBreakfastTimeProperty { 
            timestamp: value
        };
        let _publish_result = self.mqttier_client.publish_structure("full/{}/property/lastBreakfastTime/setValue".to_string(), &data);
        Ok(())
    }
    ...
    /// This returns a watch::Receiver that can be awaited on for changes to the property value.
    pub fn watch_breakfast_length(&self) -> watch::Receiver<Option<std::time::Duration>> {
        self.properties.breakfast_length_tx_channel.subscribe()
    }
    
    pub fn set_breakfast_length(&mut self, value: std::time::Duration) -> Result<(), MethodReturnCode> {
        let data = BreakfastLengthProperty { 
            length: value
        };
        let _publish_result = self.mqttier_client.publish_structure("full/{}/property/breakfastLength/setValue".to_string(), &data);
        Ok(())
    }
    ...
    pub fn set_last_birthdays(&mut self, value: LastBirthdaysProperty) -> Result<(), MethodReturnCode> {
        let data = value;
        let _publish_result = self.mqttier_client.publish_structure("full/{}/property/lastBirthdays/setValue".to_string(), &data);
        Ok(())
    }
        ...
        let _loop_task = tokio::spawn(async move {
            while let Some(msg) = message_receiver.recv().await {
                ...
                if msg.subscription_id == sub_ids.add_numbers_method_resp {
                    ...
                    FullClient::handle_what_time_is_it_response(resp_map.clone(), payload, opt_corr_id);
                }
                else if msg.subscription_id == sub_ids.set_the_time_method_resp {
                    FullClient::handle_set_the_time_response(resp_map.clone(), payload, opt_corr_id);
                }
                else if msg.subscription_id == sub_ids.forward_time_method_resp {
                    FullClient::handle_forward_time_response(resp_map.clone(), payload, opt_corr_id);
                }
                else if msg.subscription_id == sub_ids.how_off_is_the_clock_method_resp {
                    FullClient::handle_how_off_is_the_clock_response(resp_map.clone(), payload, opt_corr_id);
                }
                ...
                if msg.subscription_id == sub_ids.favorite_number_property_value {
                    ...
                    let deserialized_data: FavoriteNumberProperty = serde_json::from_slice(&msg.payload).expect("Failed to deserialize");
                    ...
                    *guard = Some(pl.clone());
                    ...
                    let deserialized_data: FamilyNameProperty = serde_json::from_slice(&msg.payload).expect("Failed to deserialize");
                    ...
                    *guard = Some(pl.clone());
                    ...
                    let deserialized_data: LastBreakfastTimeProperty = serde_json::from_slice(&msg.payload).expect("Failed to deserialize");
                    let pl = deserialized_data.timestamp;
                    
                    let mut guard = props.last_breakfast_time.lock().expect("Mutex was poisoned");
                    *guard = Some(pl.clone());
                    // Notify any watchers of the property that it has changed.
                    ...
                    let deserialized_data: BreakfastLengthProperty = serde_json::from_slice(&msg.payload).expect("Failed to deserialize");
                    ...
                    *guard = Some(pl.clone());
                    ...
                    let pl: LastBirthdaysProperty =  serde_json::from_slice(&msg.payload).expect("Failed to deserialize");
