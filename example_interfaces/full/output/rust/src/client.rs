...
impl FullClient {
    ...
    pub async fn do_something(&mut self, a_string: String)->Result<DoSomethingReturnValue, MethodReturnCode> {
        ...
        let resp_obj = receiver.await.unwrap();
        TEMPLATE ERROR CASE NOT HANDLED for multiple
    }
    ...
    ///
    /// This method awaits on the response to the call before returning.
    pub async fn forward_time(&mut self, adjustment: chrono::Duration)->Result<chrono::DateTime<chrono::Utc>, MethodReturnCode> {
        let correlation_id = Uuid::new_v4();
        let correlation_data = correlation_id.as_bytes().to_vec();
        let (sender, receiver) = oneshot::channel();
        {
    ...
    ///
    /// This method awaits on the response to the call before returning.
    pub async fn how_off_is_the_clock(&mut self, actual_time: chrono::DateTime<chrono::Utc>)->Result<chrono::Duration, MethodReturnCode> {
        let correlation_id = Uuid::new_v4();
        let correlation_data = correlation_id.as_bytes().to_vec();
        let (sender, receiver) = oneshot::channel();
        {
    ...
    pub fn set_breakfast_length(&mut self, value: chrono::Duration) -> Result<(), MethodReturnCode> {
