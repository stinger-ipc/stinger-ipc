...
impl fmt::Display for FullDiscoveryError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            FullDiscoveryError::Subscribe(err) => write!(f, "failed to subscribe for discovery: {err}"),
        }
    }
}
...
impl From<MqttierError> for FullDiscoveryError {
...
impl FullDiscovery {
    ...
    ) -> Result<Self, FullDiscoveryError> {
    ...
    fn handle_message(
        ...
        } else if let Ok(info) = serde_json::from_slice::<InterfaceInfo>(&message.payload) {
            let mut interfaces_guard = discovered_interfaces
                .write()
                .expect("interfaces write lock poisoned");
