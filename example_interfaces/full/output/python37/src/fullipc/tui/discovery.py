...
from fullipc.tui.helpers import get_package_version
import logging
...
class DiscoveryScreen(Screen):
    """Screen for discovering available FullIPC servers."""
    ...
    def on_mount(self) -> None:
        ...
        self.discoverer = FullClientDiscoverer(conn)
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if hasattr(event.button, "instance_id"):
            ...
            # Create the FullClient
            conn = self.app.mqtt_connection
            client = FullClient(conn, discovered_instance)

            # Store the client in the app for the Client screen to use
            self.app.full_client = client
