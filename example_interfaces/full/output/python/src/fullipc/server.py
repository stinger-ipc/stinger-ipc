...
class FullServer:
    ...
    def __init__(self, connection: IBrokerConnection, instance_id: str, initial_property_values: FullInitialPropertyValues):
        ...
        self._property_last_breakfast_time: PropertyControls[datetime] = PropertyControls(value=initial_property_values.last_breakfast_time, version=initial_property_values.last_breakfast_time_version)
    ...
    def _receive_favorite_number_update_request_message(self, topic: str, payload: str, properties: Dict[str, Any]):
        ...
        try:
            if int(prop_version) != int(self._property_favorite_number.version):
                ...
                if response_topic is not None:
                    self._conn.publish_property_response(response_topic, existing_prop_obj, str(self._property_favorite_number.version), MethodReturnCode.OUT_OF_SYNC, correlation_id, f"Request version {prop_version} does not match current version {self._property_favorite_number.version}")
                return
    ...
    def _receive_favorite_foods_update_request_message(self, topic: str, payload: str, properties: Dict[str, Any]):
        ...
        try:
            if int(prop_version) != int(self._property_favorite_foods.version):
                ...
                if response_topic is not None:
                    self._conn.publish_property_response(response_topic, existing_prop_obj, str(self._property_favorite_foods.version), MethodReturnCode.OUT_OF_SYNC, correlation_id, f"Request version {prop_version} does not match current version {self._property_favorite_foods.version}")
                return
            ...
            try:
                ...
                if response_topic is not None:
                    self._conn.publish_property_response(response_topic, existing_prop_obj, str(self._property_favorite_foods.version), MethodReturnCode.SERVER_DESERIALIZATION_ERROR, correlation_id, str(e))
                return
            with self._property_favorite_foods.mutex:
    ...
    def _receive_lunch_menu_update_request_message(self, topic: str, payload: str, properties: Dict[str, Any]):
        ...
        try:
            if int(prop_version) != int(self._property_lunch_menu.version):
                ...
                if response_topic is not None:
                    self._conn.publish_property_response(response_topic, existing_prop_obj, str(self._property_lunch_menu.version), MethodReturnCode.OUT_OF_SYNC, correlation_id, f"Request version {prop_version} does not match current version {self._property_lunch_menu.version}")
                return
            ...
            try:
                ...
                self._logger.error("Failed to validate payload for %s: %s", topic, e)
                if response_topic is not None:
                    self._conn.publish_property_response(response_topic, existing_prop_obj, str(self._property_lunch_menu.version), MethodReturnCode.SERVER_DESERIALIZATION_ERROR, correlation_id, str(e))
                return
            with self._property_lunch_menu.mutex:
    ...
    def _receive_family_name_update_request_message(self, topic: str, payload: str, properties: Dict[str, Any]):
        ...
        try:
            if int(prop_version) != int(self._property_family_name.version):
                ...
                if response_topic is not None:
                    self._conn.publish_property_response(response_topic, existing_prop_obj, str(self._property_family_name.version), MethodReturnCode.OUT_OF_SYNC, correlation_id, f"Request version {prop_version} does not match current version {self._property_family_name.version}")
                return
    ...
    def _receive_last_breakfast_time_update_request_message(self, topic: str, payload: str, properties: Dict[str, Any]):
        ...
        try:
            if int(prop_version) != int(self._property_last_breakfast_time.version):
                ...
                if response_topic is not None:
                    self._conn.publish_property_response(response_topic, existing_prop_obj, str(self._property_last_breakfast_time.version), MethodReturnCode.OUT_OF_SYNC, correlation_id, f"Request version {prop_version} does not match current version {self._property_last_breakfast_time.version}")
                return
    ...
    def _receive_breakfast_length_update_request_message(self, topic: str, payload: str, properties: Dict[str, Any]):
        ...
        try:
            if int(prop_version) != int(self._property_breakfast_length.version):
                ...
                if response_topic is not None:
                    self._conn.publish_property_response(response_topic, existing_prop_obj, str(self._property_breakfast_length.version), MethodReturnCode.OUT_OF_SYNC, correlation_id, f"Request version {prop_version} does not match current version {self._property_breakfast_length.version}")
                return
    ...
    def _receive_last_birthdays_update_request_message(self, topic: str, payload: str, properties: Dict[str, Any]):
        ...
        try:
            if int(prop_version) != int(self._property_last_birthdays.version):
                ...
                if response_topic is not None:
                    self._conn.publish_property_response(response_topic, existing_prop_obj, str(self._property_last_birthdays.version), MethodReturnCode.OUT_OF_SYNC, correlation_id, f"Request version {prop_version} does not match current version {self._property_last_birthdays.version}")
                return
            ...
            try:
                ...
                self._logger.error("Failed to validate payload for %s: %s", topic, e)
                if response_topic is not None:
                    self._conn.publish_property_response(response_topic, existing_prop_obj, str(self._property_last_birthdays.version), MethodReturnCode.SERVER_DESERIALIZATION_ERROR, correlation_id, str(e))
                return
            with self._property_last_birthdays.mutex:
