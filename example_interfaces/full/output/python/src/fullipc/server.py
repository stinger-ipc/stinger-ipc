...
class FullServer:
    ...
    def __init__(self, connection: IBrokerConnection, instance_id: str, initial_property_values: FullInitialPropertyValues):
        ...
        self._property_favorite_number: PropertyControls[int] = PropertyControls()
        self._property_favorite_number.subscription_id = self._conn.subscribe("full/{}/property/favoriteNumber/setValue".format(self._instance_id), self._receive_favorite_number_update_request_message)
        
        self._property_favorite_foods: PropertyControls[FavoriteFoodsProperty] = PropertyControls()
        ...
        self._property_lunch_menu: PropertyControls[LunchMenuProperty] = PropertyControls()
        ...
        self._property_family_name: PropertyControls[str] = PropertyControls()
        ...
        self._property_last_breakfast_time.subscription_id = self._conn.subscribe("full/{}/property/lastBreakfastTime/setValue".format(self._instance_id), self._receive_last_breakfast_time_update_request_message)
        
        self._property_breakfast_length: PropertyControls[timedelta] = PropertyControls()
        self._property_breakfast_length.subscription_id = self._conn.subscribe("full/{}/property/breakfastLength/setValue".format(self._instance_id), self._receive_breakfast_length_update_request_message)
        
        self._property_last_birthdays: PropertyControls[LastBirthdaysProperty] = PropertyControls()
    ...
    def _receive_favorite_number_update_request_message(self, topic: str, payload: str, properties: Dict[str, Any]):
        ...
        try:
            if prop_version != self._property_favorite_number.version:
                self._logger.warning("Received out-of-date update for %s (version %s, current version %s)", topic, prop_version, self._property_favorite_number.version)
                if response_topic is not None:
                    self._conn.publish_property_response(response_topic, existing_prop_obj, self._property_favorite_number.version, MethodReturnCode.OUT_OF_SYNC, correlation_id, f"Request version {prop_version} does not match current version {self._property_favorite_number.version}")
                return
            ...
            if response_topic is not None:
                ...
                self._conn.publish_property_response(response_topic, prop_obj, self._property_favorite_number.version, MethodReturnCode.SUCCESS, correlation_id)
            else:
            ...
            if response_topic is not None:
                ...
                self._conn.publish_property_response(response_topic, prop_obj, self._property_favorite_number.version, MethodReturnCode.SERVER_ERROR, correlation_id, str(e))
    ...
    def _receive_favorite_foods_update_request_message(self, topic: str, payload: str, properties: Dict[str, Any]):
        ...
        try:
            if prop_version != self._property_favorite_foods.version:
                self._logger.warning("Received out-of-date update for %s (version %s, current version %s)", topic, prop_version, self._property_favorite_foods.version)
                if response_topic is not None:
                    self._conn.publish_property_response(response_topic, existing_prop_obj, self._property_favorite_foods.version, MethodReturnCode.OUT_OF_SYNC, correlation_id, f"Request version {prop_version} does not match current version {self._property_favorite_foods.version}")
                return
            ...
            try:
                ...
                if response_topic is not None:
                self._conn.publish_property_response(response_topic, existing_prop_obj, self._property_favorite_foods.version, MethodReturnCode.SERVER_DESERIALIZATION_ERROR, correlation_id, str(e))
                return
            ...
            if response_topic is not None:
                ...
                self._conn.publish_property_response(response_topic, prop_obj, self._property_favorite_foods.version, MethodReturnCode.SUCCESS, correlation_id)
            else:
                self._logger.warning("No response topic provided for property update of %s", topic)
            ...
            if response_topic is not None:
                ...
                self._conn.publish_property_response(response_topic, prop_obj, self._property_favorite_foods.version, MethodReturnCode.SERVER_ERROR, correlation_id, str(e))
    ...
    def _receive_lunch_menu_update_request_message(self, topic: str, payload: str, properties: Dict[str, Any]):
        ...
        try:
            if prop_version != self._property_lunch_menu.version:
                ...
                if response_topic is not None:
                    self._conn.publish_property_response(response_topic, existing_prop_obj, self._property_lunch_menu.version, MethodReturnCode.OUT_OF_SYNC, correlation_id, f"Request version {prop_version} does not match current version {self._property_lunch_menu.version}")
                return
            ...
            try:
                ...
                self._logger.error("Failed to validate payload for %s: %s", topic, e)
                if response_topic is not None:
                self._conn.publish_property_response(response_topic, existing_prop_obj, self._property_lunch_menu.version, MethodReturnCode.SERVER_DESERIALIZATION_ERROR, correlation_id, str(e))
                return
            ...
            if response_topic is not None:
                ...
                self._conn.publish_property_response(response_topic, prop_obj, self._property_lunch_menu.version, MethodReturnCode.SUCCESS, correlation_id)
            else:
                self._logger.warning("No response topic provided for property update of %s", topic)
            ...
            if response_topic is not None:
                ...
                self._conn.publish_property_response(response_topic, prop_obj, self._property_lunch_menu.version, MethodReturnCode.SERVER_ERROR, correlation_id, str(e))
    ...
    def _receive_family_name_update_request_message(self, topic: str, payload: str, properties: Dict[str, Any]):
        ...
        try:
            if prop_version != self._property_family_name.version:
                self._logger.warning("Received out-of-date update for %s (version %s, current version %s)", topic, prop_version, self._property_family_name.version)
                if response_topic is not None:
                    self._conn.publish_property_response(response_topic, existing_prop_obj, self._property_family_name.version, MethodReturnCode.OUT_OF_SYNC, correlation_id, f"Request version {prop_version} does not match current version {self._property_family_name.version}")
                return
            ...
            if response_topic is not None:
                ...
                self._conn.publish_property_response(response_topic, prop_obj, self._property_family_name.version, MethodReturnCode.SUCCESS, correlation_id)
            else:
                self._logger.warning("No response topic provided for property update of %s", topic)
            ...
            if response_topic is not None:
                ...
                self._conn.publish_property_response(response_topic, prop_obj, self._property_family_name.version, MethodReturnCode.SERVER_ERROR, correlation_id, str(e))
    ...
    def _receive_last_breakfast_time_update_request_message(self, topic: str, payload: str, properties: Dict[str, Any]):
        ...
        try:
            if prop_version != self._property_last_breakfast_time.version:
                self._logger.warning("Received out-of-date update for %s (version %s, current version %s)", topic, prop_version, self._property_last_breakfast_time.version)
                if response_topic is not None:
                    self._conn.publish_property_response(response_topic, existing_prop_obj, self._property_last_breakfast_time.version, MethodReturnCode.OUT_OF_SYNC, correlation_id, f"Request version {prop_version} does not match current version {self._property_last_breakfast_time.version}")
                return
            ...
            if response_topic is not None:
                ...
                self._conn.publish_property_response(response_topic, prop_obj, self._property_last_breakfast_time.version, MethodReturnCode.SUCCESS, correlation_id)
            else:
                self._logger.warning("No response topic provided for property update of %s", topic)
            ...
            if response_topic is not None:
                ...
                self._conn.publish_property_response(response_topic, prop_obj, self._property_last_breakfast_time.version, MethodReturnCode.SERVER_ERROR, correlation_id, str(e))
    ...
    def _receive_breakfast_length_update_request_message(self, topic: str, payload: str, properties: Dict[str, Any]):
        ...
        try:
            if prop_version != self._property_breakfast_length.version:
                ...
                if response_topic is not None:
                    self._conn.publish_property_response(response_topic, existing_prop_obj, self._property_breakfast_length.version, MethodReturnCode.OUT_OF_SYNC, correlation_id, f"Request version {prop_version} does not match current version {self._property_breakfast_length.version}")
                return
            ...
            if response_topic is not None:
                ...
                self._conn.publish_property_response(response_topic, prop_obj, self._property_breakfast_length.version, MethodReturnCode.SUCCESS, correlation_id)
            else:
                self._logger.warning("No response topic provided for property update of %s", topic)
            ...
            if response_topic is not None:
                ...
                self._conn.publish_property_response(response_topic, prop_obj, self._property_breakfast_length.version, MethodReturnCode.SERVER_ERROR, correlation_id, str(e))
    ...
    def _receive_last_birthdays_update_request_message(self, topic: str, payload: str, properties: Dict[str, Any]):
        ...
        try:
            if prop_version != self._property_last_birthdays.version:
                self._logger.warning("Received out-of-date update for %s (version %s, current version %s)", topic, prop_version, self._property_last_birthdays.version)
                if response_topic is not None:
                    self._conn.publish_property_response(response_topic, existing_prop_obj, self._property_last_birthdays.version, MethodReturnCode.OUT_OF_SYNC, correlation_id, f"Request version {prop_version} does not match current version {self._property_last_birthdays.version}")
                return
            ...
            try:
                ...
                self._logger.error("Failed to validate payload for %s: %s", topic, e)
                if response_topic is not None:
                self._conn.publish_property_response(response_topic, existing_prop_obj, self._property_last_birthdays.version, MethodReturnCode.SERVER_DESERIALIZATION_ERROR, correlation_id, str(e))
                return
            ...
            if response_topic is not None:
                ...
                self._conn.publish_property_response(response_topic, prop_obj, self._property_last_birthdays.version, MethodReturnCode.SUCCESS, correlation_id)
            else:
                self._logger.warning("No response topic provided for property update of %s", topic)
            ...
            if response_topic is not None:
                ...
                self._conn.publish_property_response(response_topic, prop_obj, self._property_last_birthdays.version, MethodReturnCode.SERVER_ERROR, correlation_id, str(e))
    ...
    def _process_add_numbers_call(self, topic: str, payload_str: str, properties: Dict[str, Any]):
        ...
        if self._method_add_numbers.callback is not None:
            method_args = [payload.first, payload.second, payload.third, ]
    ...
    def _process_what_time_is_it_call(self, topic: str, payload_str: str, properties: Dict[str, Any]):
        ...
        if self._method_what_time_is_it.callback is not None:
            method_args = [payload.the_first_time, ]
    ...
    def _process_set_the_time_call(self, topic: str, payload_str: str, properties: Dict[str, Any]):
        ...
        if self._method_set_the_time.callback is not None:
            method_args = [payload.the_first_time, payload.the_second_time, ]
    ...
    def _process_forward_time_call(self, topic: str, payload_str: str, properties: Dict[str, Any]):
        ...
        if self._method_forward_time.callback is not None:
            method_args = [payload.adjustment, ]
    ...
    def _process_how_off_is_the_clock_call(self, topic: str, payload_str: str, properties: Dict[str, Any]):
        ...
        if self._method_how_off_is_the_clock.callback is not None:
            method_args = [payload.actual_time, ]
