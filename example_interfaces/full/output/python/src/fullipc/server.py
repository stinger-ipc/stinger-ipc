...
class FullServer:
    ...
    def publish_last_breakfast_time_value(self, *_, **__):
        ...
        with self._property_last_breakfast_time.mutex:
            ...
            state_msg = MessageCreator.property_state_message("full/{}/property/lastBreakfastTime/value".format(self._instance_id), last_breakfast_time_prop_obj, self._property_last_breakfast_time.version)
            self._conn.publish(state_msg)
    ...
    def _receive_favorite_number_update_request_message(self, message: Message):
        ...
        try:
            ...
            for favorite_number_callback in self._property_favorite_number.callbacks:
                
                favorite_number_callback(current_prop_obj.number)
    ...
    def _receive_favorite_foods_update_request_message(self, message: Message):
        ...
        try:
            ...
            for favorite_foods_callback in self._property_favorite_foods.callbacks:
                ...
                favorite_foods_callback(current_prop_obj)
    ...
    def _receive_family_name_update_request_message(self, message: Message):
        ...
        try:
            ...
            for family_name_callback in self._property_family_name.callbacks:
                
                family_name_callback(current_prop_obj.family_name)
    ...
    def _receive_last_breakfast_time_update_request_message(self, message: Message):
        ...
        try:
            ...
            for last_breakfast_time_callback in self._property_last_breakfast_time.callbacks:
                
                last_breakfast_time_callback(current_prop_obj.timestamp)
    ...
    def _receive_last_birthdays_update_request_message(self, message: Message):
        ...
        try:
            ...
            for last_birthdays_callback in self._property_last_birthdays.callbacks:
                ...
                last_birthdays_callback(current_prop_obj)
    ...
    def favorite_number(self, number: int):
        ...
        if value_updated:
            for favorite_number_callback in self._property_favorite_number.callbacks:
                favorite_number_callback(prop_obj.number)
        ...
        if not isinstance(value, FavoriteFoodsProperty):
            ...
            with self._property_favorite_foods.mutex:
                if value != self._property_favorite_foods.get_value():
                    ...
                    self._property_favorite_foods.version += 1
                    state_msg = MessageCreator.property_state_message("full/{}/property/favoriteFoods/value".format(self._instance_id), self._property_favorite_foods.get_value(), self._property_favorite_foods.version)
                    self._conn.publish(state_msg)
        ...
        if not isinstance(value, LunchMenuProperty):
            ...
            with self._property_lunch_menu.mutex:
                if value != self._property_lunch_menu.get_value():
                    ...
                    self._property_lunch_menu.version += 1
                    state_msg = MessageCreator.property_state_message("full/{}/property/lunchMenu/value".format(self._instance_id), self._property_lunch_menu.get_value(), self._property_lunch_menu.version)
                    self._conn.publish(state_msg)
    ...
    def family_name(self, family_name: str):
        ...
        if value_updated:
            for family_name_callback in self._property_family_name.callbacks:
                family_name_callback(prop_obj.family_name)
    ...
    def last_breakfast_time(self, timestamp: datetime):
        ...
        if value_updated:
            for last_breakfast_time_callback in self._property_last_breakfast_time.callbacks:
                last_breakfast_time_callback(prop_obj.timestamp)
        ...
        if not isinstance(value, LastBirthdaysProperty):
            ...
            with self._property_last_birthdays.mutex:
                if value != self._property_last_birthdays.get_value():
                    ...
                    self._property_last_birthdays.version += 1
                    state_msg = MessageCreator.property_state_message("full/{}/property/lastBirthdays/value".format(self._instance_id), self._property_last_birthdays.get_value(), self._property_last_birthdays.version)
                    self._conn.publish(state_msg)
