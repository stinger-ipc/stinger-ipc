...
class FullServer:
    ...
    def __init__(self, connection: BrokerConnection):
        ...
        self._conn.set_last_will(
            topic="Full/interface", payload=None, qos=1, retain=True
        )
        ...
        self._add_numbers_method_handler: Optional[
            Callable[[int, int, int | None], int]
        ] = None
        ...
        if self._conn.is_topic_sub(topic, "Full/method/addNumbers"):
            try:
                ...
                self._logger.warning(
                    "Invalid JSON payload received at topic '%s'", topic
                )
            else:
    ...
    def emit_todayIs(
        self, dayOfMonth: int, dayOfWeek: stinger_types.DayOfTheWeek | None
    ):
        ...
        if (
            not isinstance(dayOfWeek, stinger_types.DayOfTheWeek)
            and dayOfWeek is not None
        ):
            raise ValueError(
                f"The 'dayOfWeek' value must be stinger_types.DayOfTheWeek | None."
            )

        payload = {
            "dayOfMonth": int(dayOfMonth),
            "dayOfWeek": (
                stinger_types.DayOfTheWeek(dayOfWeek).value
                if dayOfWeek is not None
                else None
            ),
        }
        self._conn.publish(
            "Full/signal/todayIs", json.dumps(payload), qos=1, retain=False
        )
    ...
    def _process_add_numbers_call(
        self, topic: str, payload: Dict[str, Any], properties: Dict[str, Any]
    ):
        ...
        if self._add_numbers_method_handler is not None:
            ...
            if "first" in payload:
                if not isinstance(payload["first"], int):
                    self._logger.warning(
                        "The 'first' property in the payload to '%s' wasn't the correct type.  It should have been int.",
                        topic,
                    )
                ...
                self._logger.warning(
                    "The 'first' property in the payload to '%s' wasn't present", topic
                )
                # TODO: return an error via MQTT
                return
            ...
            if "second" in payload:
                if not isinstance(payload["second"], int):
                    self._logger.warning(
                        "The 'second' property in the payload to '%s' wasn't the correct type.  It should have been int.",
                        topic,
                    )
                    # TODO: return an error via MQTT
                    return
                ...
                self._logger.warning(
                    "The 'second' property in the payload to '%s' wasn't present", topic
                )
                # TODO: return an error via MQTT
                return
            ...
            if "third" in payload:
                if not isinstance(payload["third"], int | None) or third is None:
                    self._logger.warning(
                        "The 'third' property in the payload to '%s' wasn't the correct type.  It should have been int | None.",
                        topic,
                    )
                    # TODO: return an error via MQTT
                    return
            ...
            if response_topic is not None:
                ...
                try:
                    ...
                    self._logger.exception(
                        "Exception while handling addNumbers", exc_info=e
                    )
    ...
    def _process_do_something_call(
        self, topic: str, payload: Dict[str, Any], properties: Dict[str, Any]
    ):
        ...
        if self._do_something_method_handler is not None:
            ...
            if "aString" in payload:
                if not isinstance(payload["aString"], str):
                    self._logger.warning(
                        "The 'aString' property in the payload to '%s' wasn't the correct type.  It should have been str.",
                        topic,
                    )
                    # TODO: return an error via MQTT
                    return
                ...
                self._logger.warning(
                    "The 'aString' property in the payload to '%s' wasn't present",
                    topic,
                )
                # TODO: return an error via MQTT
                return
            ...
            if response_topic is not None:
                ...
                try:
                    ...
                    self._logger.exception(
                        "Exception while handling doSomething", exc_info=e
                    )
                    return_code = MethodResultCode.SERVER_ERROR
                    debug_msg = str(e)
                else:
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            sleep(4)
            server.emit_todayIs(
                dayOfMonth=42, dayOfWeek=stinger_types.DayOfTheWeek.MONDAY
            )
