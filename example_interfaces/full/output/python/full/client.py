...
FavoriteFoodsPropertyUpdatedCallbackType = Callable[
    [stinger_types.FavoriteFoodsProperty], None
]
...
class FullClient:
    ...
    def __init__(self, connection: BrokerConnection):
        ...
        self._favorite_number_prop_subscription_id: int = self._conn.subscribe(
            "Full/property/favorite_number"
        )
        self._changed_value_callbacks_for_favorite_number: list[
            FavoriteNumberPropertyUpdatedCallbackType
        ] = []
        self._property_favorite_foods: stinger_types.FavoriteFoodsProperty | None = None
        self._favorite_foods_prop_subscription_id: int = self._conn.subscribe(
            "Full/property/favorite_foods"
        )
        self._changed_value_callbacks_for_favorite_foods: list[
            FavoriteFoodsPropertyUpdatedCallbackType
        ] = []
        self._property_lunch_menu: stinger_types.LunchMenuProperty | None = None
        self._lunch_menu_prop_subscription_id: int = self._conn.subscribe(
            "Full/property/lunch_menu"
        )
        self._changed_value_callbacks_for_lunch_menu: list[
            LunchMenuPropertyUpdatedCallbackType
        ] = []
        self._signal_recv_callbacks_for_today_is: list[TodayIsSignalCallbackType] = []
        self._addNumbers_method_call_subscription_id: int = self._conn.subscribe(
            f"client/{self._client_id}/Full/method/addNumbers/response"
        )
        self._doSomething_method_call_subscription_id: int = self._conn.subscribe(
            f"client/{self._client_id}/Full/method/doSomething/response"
        )
    ...
    def favorite_number(self, value: int):
        ...
        self._conn.publish("Full/property/favorite_number/set_value", serialized, qos=1)

    def favorite_number_changed(
        self,
        handler: FavoriteNumberPropertyUpdatedCallbackType,
    ...
    def favorite_foods(self, value: stinger_types.FavoriteFoodsProperty):
        ...
        if not isinstance(value, stinger_types.FavoriteFoodsProperty):
            raise ValueError(
                "The 'favorite_foods' property must be a stinger_types.FavoriteFoodsProperty"
            )
        serialized = value.model_dump_json(exclude_none=True)
        self._logger.debug("Setting 'favorite_foods' property to %s", serialized)
        self._conn.publish("Full/property/favorite_foods/set_value", serialized, qos=1)

    def favorite_foods_changed(
        self,
        handler: FavoriteFoodsPropertyUpdatedCallbackType,
        call_immediately: bool = False,
    ):
    ...
    def lunch_menu(self, value: stinger_types.LunchMenuProperty):
        ...
        if not isinstance(value, stinger_types.LunchMenuProperty):
            raise ValueError(
                "The 'lunch_menu' property must be a stinger_types.LunchMenuProperty"
            )
        serialized = value.model_dump_json(exclude_none=True)
        self._logger.debug("Setting 'lunch_menu' property to %s", serialized)
        self._conn.publish("Full/property/lunch_menu/set_value", serialized, qos=1)

    def lunch_menu_changed(
        self,
        handler: LunchMenuPropertyUpdatedCallbackType,
        call_immediately: bool = False,
    ):
    ...
    def _receive_message(self, topic: str, payload: str, properties: Dict[str, Any]):
        ...
        if self._conn.is_topic_sub(topic, "Full/signal/todayIs"):
            if (
                ...
                self._logger.warning(
                    "Received 'todayIs' signal with non-JSON content type"
                )
                return
            allowed_args = [
                "dayOfMonth",
            ...
            kwargs["dayOfWeek"] = (
                stinger_types.DayOfTheWeek(kwargs["dayOfWeek"])
                if kwargs.get("dayOfWeek")
                else None
            )
        ...
        if self._conn.is_topic_sub(
            topic, f"client/{self._client_id}/Full/method/addNumbers/response"
        ):
            ...
            if "UserProperty" in properties:
                ...
                if "DebugInfo" in user_properties:
                    self._logger.info(
                        "Received Debug Info: %s", user_properties["DebugInfo"]
                    )
            ...
            if "CorrelationData" in properties:
                ...
                if correlation_id in self._pending_method_responses:
                    ...
                    self._logger.warning(
                        "Correlation id %s was not in the list of pending method responses... %s",
                        correlation_id,
                        [k for k in self._pending_method_responses.keys()],
                    )
            else:
                self._logger.warning(
                    "No correlation data in properties sent to %s... %s",
                    topic,
                    [s for s in properties.keys()],
                )

        # Handle 'doSomething' method response.
        if self._conn.is_topic_sub(
            topic, f"client/{self._client_id}/Full/method/doSomething/response"
        ):
            result_code = MethodResultCode.SUCCESS
            if "UserProperty" in properties:
                user_properties = properties["UserProperty"]
        ...
        if self._conn.is_topic_sub(topic, "Full/property/favorite_number"):
            if (
                "ContentType" not in properties
                or properties["ContentType"] != "application/json"
            ):
                self._logger.warning(
                    "Received 'favorite_number' property change with non-JSON content type"
                )
                return
            try:
                ...
                self._do_callbacks_for(
                    self._changed_value_callbacks_for_favorite_number,
                    value=self._property_favorite_number,
                )
            except Exception as e:
                self._logger.error(
                    "Error processing 'favorite_number' property change: %s", e
                )

        # Handle 'favorite_foods' property change.
        elif self._conn.is_topic_sub(topic, "Full/property/favorite_foods"):
            if (
                "ContentType" not in properties
                or properties["ContentType"] != "application/json"
            ):
                self._logger.warning(
                    "Received 'favorite_foods' property change with non-JSON content type"
                )
                return
            try:
                prop_value = stinger_types.FavoriteFoodsProperty.model_validate_json(
                    payload
                )
                self._property_favorite_foods = prop_value
                self._do_callbacks_for(
                    self._changed_value_callbacks_for_favorite_foods,
                    value=self._property_favorite_foods,
                )
            except Exception as e:
                self._logger.error(
                    "Error processing 'favorite_foods' property change: %s", e
                )

        # Handle 'lunch_menu' property change.
        elif self._conn.is_topic_sub(topic, "Full/property/lunch_menu"):
            if (
                "ContentType" not in properties
                or properties["ContentType"] != "application/json"
            ):
                self._logger.warning(
                    "Received 'lunch_menu' property change with non-JSON content type"
                )
                return
            try:
                prop_value = stinger_types.LunchMenuProperty.model_validate_json(
                    payload
                )
                self._property_lunch_menu = prop_value
                self._do_callbacks_for(
                    self._changed_value_callbacks_for_lunch_menu,
                    value=self._property_lunch_menu,
                )
            except Exception as e:
                self._logger.error(
                    "Error processing 'lunch_menu' property change: %s", e
                )
    ...
    def add_numbers(self, first: int, second: int, third: int | None) -> futures.Future:
        ...
        self._pending_method_responses[correlation_id] = partial(
            self._handle_add_numbers_response, fut
        )
        payload = {
            "first": first,
            "second": second,
        ...
        self._conn.publish(
            "Full/method/addNumbers",
            json.dumps(payload),
            qos=2,
            retain=False,
            correlation_id=correlation_id,
            response_topic=f"client/{self._client_id}/Full/method/addNumbers/response",
        )
        return fut

    def _handle_add_numbers_response(
        self,
        fut: futures.Future,
        response_json: Dict[str, Any],
        return_value: MethodResultCode,
    ):
        ...
        try:
            if return_value != MethodResultCode.SUCCESS.value:
                raise stinger_exception_factory(
                    return_value,
                    (
                        response_json["debugResultMessage"]
                        if "debugResultMessage" in response_json
                        else None
                    ),
                )
    ...
    def do_something(self, aString: str) -> futures.Future:
        ...
        correlation_id = str(uuid4())
        self._pending_method_responses[correlation_id] = partial(
            self._handle_do_something_response, fut
        )
        payload = {
            "aString": aString,
        }
        self._conn.publish(
            "Full/method/doSomething",
            json.dumps(payload),
            qos=2,
            retain=False,
            correlation_id=correlation_id,
            response_topic=f"client/{self._client_id}/Full/method/doSomething/response",
        )
        return fut

    def _handle_do_something_response(
        self,
        fut: futures.Future,
        response_json: Dict[str, Any],
        return_value: MethodResultCode,
...
class FullClientBuilder:
    ...
    def __init__(self, broker: BrokerConnection):
        ...
        self._signal_recv_callbacks_for_today_is = (
            []
        )  # type: List[TodayIsSignalCallbackType]
        self._property_updated_callbacks_for_favorite_number: list[
            FavoriteNumberPropertyUpdatedCallbackType
        ] = []
        self._property_updated_callbacks_for_favorite_foods: list[
            FavoriteFoodsPropertyUpdatedCallbackType
        ] = []
        self._property_updated_callbacks_for_lunch_menu: list[
            LunchMenuPropertyUpdatedCallbackType
        ] = []
    ...
    def favorite_number_updated(
        self, handler: FavoriteNumberPropertyUpdatedCallbackType
    ):
...
if __name__ == "__main__":
    ...
    def print_todayIs_receipt(
        dayOfMonth: int, dayOfWeek: stinger_types.DayOfTheWeek | None
    ):
        """
        ...
        """
        print(
            f"Got a 'todayIs' signal: dayOfMonth={ dayOfMonth } dayOfWeek={ dayOfWeek } "
        )
