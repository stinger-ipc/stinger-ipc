...
WeatherDiscovery::WeatherDiscovery(std::shared_ptr<IBrokerConnection> broker): _broker(broker)
{
    // Subscribe to the discovery topic
    std::stringstream topicStream;
    topicStream << boost::format("weather/%1%/interface") % "+";
...
void WeatherDiscovery::SetDiscoveryCallback(const std::function<void(const std::string&)>& cb)
{
    std::lock_guard<std::mutex> lock(_mutex);
    _discovery_callback = cb;
}

boost::future<std::string> WeatherDiscovery::GetSingleton()
{
    std::lock_guard<std::mutex> lock(_mutex);
...
std::vector<std::string> WeatherDiscovery::GetInstanceIds() const
{
    std::lock_guard<std::mutex> lock(_mutex);
    return _instance_ids;
}

void WeatherDiscovery::_onMessage(const std::string& topic, const std::string& payload, const MqttProperties& mqttProps)
{
    // Check if this message is for our discovery topic
    std::stringstream topicPattern;
    topicPattern << boost::format("weather/%1%/interface") % "+";
