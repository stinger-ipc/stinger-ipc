...
pub trait WeatherServerMethodHandler: Send + Sync {
    /// Pointer to a function to handle the refresh_daily_forecast method request.
    handle_refresh_daily_forecast(&self, ) -> Result<(), MethodResultCode>;
    
    /// Pointer to a function to handle the refresh_hourly_forecast method request.
    handle_refresh_hourly_forecast(&self, ) -> Result<(), MethodResultCode>;
    
    /// Pointer to a function to handle the refresh_current_conditions method request.
    handle_refresh_current_conditions(&self, ) -> Result<(), MethodResultCode>;
