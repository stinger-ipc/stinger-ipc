...
struct WeatherProperties {
    ...
    location: Arc<Mutex<Option<LocationProperty>>>,current_temperature_topic: Arc<String>,
    current_temperature: Arc<Mutex<Option<f32>>>,
    current_condition_topic: Arc<String>,
    current_condition: Arc<Mutex<Option<CurrentConditionProperty>>>,daily_forecast_topic: Arc<String>,
    daily_forecast: Arc<Mutex<Option<DailyForecastProperty>>>,hourly_forecast_topic: Arc<String>,
    hourly_forecast: Arc<Mutex<Option<HourlyForecastProperty>>>,current_condition_refresh_interval_topic: Arc<String>,
...
impl WeatherServer {
    ...
    pub async fn emit_current_time(&mut self, current_time: String) {
        ...
        let _ = self.mqttier_client.publish_state("weather/signal/currentTime".to_string(), &data, 1).await;
    }
    ...
    async fn handle_refresh_daily_forecast_request(publisher: MqttierClient, handlers: &mut WeatherServerMethodHandlers, msg: ReceivedMessage) {
    ...
    async fn handle_refresh_hourly_forecast_request(publisher: MqttierClient, handlers: &mut WeatherServerMethodHandlers, msg: ReceivedMessage) {
        let opt_corr_data = msg.correlation_data;
        let opt_resp_topic = msg.response_topic;
    ...
    async fn handle_refresh_current_conditions_request(publisher: MqttierClient, handlers: &mut WeatherServerMethodHandlers, msg: ReceivedMessage) {
        let opt_corr_data = msg.correlation_data;
        let opt_resp_topic = msg.response_topic;
    ...
    async fn publish_location_value(publisher: MqttierClient, topic: String, data: LocationProperty)
    {
        let _pub_result = publisher.publish_state(topic, &data, 1).await;
        
    }
    
    async fn update_location_value(publisher: MqttierClient, topic: Arc<String>, data: Arc<Mutex<Option<LocationProperty>>>, msg: ReceivedMessage)
    {
    ...
    async fn publish_current_condition_value(publisher: MqttierClient, topic: String, data: CurrentConditionProperty)
    {
        let _pub_result = publisher.publish_state(topic, &data, 1).await;
    ...
    async fn publish_daily_forecast_value(publisher: MqttierClient, topic: String, data: DailyForecastProperty)
    {
        let _pub_result = publisher.publish_state(topic, &data, 1).await;
    ...
    async fn publish_hourly_forecast_value(publisher: MqttierClient, topic: String, data: HourlyForecastProperty)
    {
        let _pub_result = publisher.publish_state(topic, &data, 1).await;
    ...
    async fn publish_current_condition_refresh_interval_value(publisher: MqttierClient, topic: String, data: i32)
    {
    ...
    async fn update_current_condition_refresh_interval_value(publisher: MqttierClient, topic: Arc<String>, data: Arc<Mutex<Option<i32>>>, msg: ReceivedMessage)
    {
        let payload_str = String::from_utf8_lossy(&msg.payload).to_string();
    ...
    async fn publish_hourly_forecast_refresh_interval_value(publisher: MqttierClient, topic: String, data: i32)
    {
    ...
    async fn update_hourly_forecast_refresh_interval_value(publisher: MqttierClient, topic: Arc<String>, data: Arc<Mutex<Option<i32>>>, msg: ReceivedMessage)
    {
        let payload_str = String::from_utf8_lossy(&msg.payload).to_string();
    ...
    async fn publish_daily_forecast_refresh_interval_value(publisher: MqttierClient, topic: String, data: i32)
    {
    ...
    async fn update_daily_forecast_refresh_interval_value(publisher: MqttierClient, topic: Arc<String>, data: Arc<Mutex<Option<i32>>>, msg: ReceivedMessage)
    {
        let payload_str = String::from_utf8_lossy(&msg.payload).to_string();
        ...
        let loop_task = tokio::spawn(async move {
            while let Some(msg) = message_receiver.recv().await {
                if msg.subscription_id == sub_ids.refresh_daily_forecast_method_req {
                    ...
                    WeatherServer::handle_refresh_hourly_forecast_request(publisher.clone(), &mut method_handlers, msg).await;
                }
                else if msg.subscription_id == sub_ids.refresh_current_conditions_method_req {
                    WeatherServer::handle_refresh_current_conditions_request(publisher.clone(), &mut method_handlers, msg).await;
                }
                else if msg.subscription_id == sub_ids.location_property_update {
                    WeatherServer::update_location_value(publisher.clone(), properties.location_topic.clone(), properties.location.clone(), msg).await;
                }
                else if msg.subscription_id == sub_ids.current_condition_refresh_interval_property_update {
                    WeatherServer::update_current_condition_refresh_interval_value(publisher.clone(), properties.current_condition_refresh_interval_topic.clone(), properties.current_condition_refresh_interval.clone(), msg).await;
                }
                else if msg.subscription_id == sub_ids.hourly_forecast_refresh_interval_property_update {
                    WeatherServer::update_hourly_forecast_refresh_interval_value(publisher.clone(), properties.hourly_forecast_refresh_interval_topic.clone(), properties.hourly_forecast_refresh_interval.clone(), msg).await;
                }
