"""Discovery screen for finding SignalOnlyIPC servers."""
...
from signalonlyipc.tui.helpers import get_package_version
import logging
...
class DiscoveryScreen(Screen):
    """Screen for discovering available SignalOnlyIPC servers."""
    ...
    def on_mount(self) -> None:
        ...
        self.discoverer = SignalOnlyClientDiscoverer(conn)
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if hasattr(event.button, "instance_id"):
            ...
            # Create the SignalOnlyClient
            conn = self.app.mqtt_connection
            client = SignalOnlyClient(conn, discovered_instance)

            # Store the client in the app for the Client screen to use
            self.app.signal_only_client = client
