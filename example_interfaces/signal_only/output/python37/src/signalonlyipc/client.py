...
class SignalOnlyClient:
    ...
    def receive_another_signal(self, handler: AnotherSignalSignalCallbackType):
        ...
        if len(self._signal_recv_callbacks_for_another_signal) == 1:
            self._conn.subscribe("{prefix}/SignalOnly/{service_id}/signal/anotherSignal".format(**self._topic_template_kwargs), self._receive_another_signal_signal_message)
        return handler
    ...
    def receive_bark(self, handler: BarkSignalCallbackType):
        ...
        if len(self._signal_recv_callbacks_for_bark) == 1:
            self._conn.subscribe("{prefix}/SignalOnly/{service_id}/signal/bark".format(**self._topic_template_kwargs), self._receive_bark_signal_message)
        return handler
    ...
    def receive_maybe_number(self, handler: MaybeNumberSignalCallbackType):
        ...
        if len(self._signal_recv_callbacks_for_maybe_number) == 1:
            self._conn.subscribe("{prefix}/SignalOnly/{service_id}/signal/maybe_number".format(**self._topic_template_kwargs), self._receive_maybe_number_signal_message)
        return handler
    ...
    def receive_maybe_name(self, handler: MaybeNameSignalCallbackType):
        ...
        if len(self._signal_recv_callbacks_for_maybe_name) == 1:
            self._conn.subscribe("{prefix}/SignalOnly/{service_id}/signal/maybe_name".format(**self._topic_template_kwargs), self._receive_maybe_name_signal_message)
        return handler
    ...
    def receive_now(self, handler: NowSignalCallbackType):
        ...
        if len(self._signal_recv_callbacks_for_now) == 1:
            self._conn.subscribe("{prefix}/SignalOnly/{service_id}/signal/now".format(**self._topic_template_kwargs), self._receive_now_signal_message)
        return handler
...
class SignalOnlyClientDiscoverer:
    ...
    def __init__(self, connection: IBrokerConnection, builder: Optional[SignalOnlyClientBuilder]=None, build_binding: Optional[Any]=None):
        ...
        self._logger.setLevel(logging.DEBUG)
        service_discovery_topic = "{prefix}/SignalOnly/{service_id}/interface".format(client_id='+', service_id='+', prefix='+', )
    ...
    def _process_service_discovery_message(self, message: Message):
        ...
        if len(message.payload) > 0:
            try:
                service_info = SignalOnlyInterfaceInfo.model_validate_json(message.payload)
