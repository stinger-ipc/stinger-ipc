...
struct SignalOnlySubscriptionIds {
    ...
    another_signal_signal: Option<usize>,bark_signal: Option<usize>,maybe_number_signal: Option<usize>,maybe_name_signal: Option<usize>,now_signal: Option<usize>,
...
impl SignalOnlyClient {
    ...
    pub async fn new(connection: &mut MqttierClient) -> Self {
        ...
        let subscription_id_now_signal = connection.subscribe(topic_now_signal, 2, message_received_tx.clone()).await;
        ...
        let _loop_task = tokio::spawn(async move {
            while let Some(msg) = message_receiver.recv().await {
                ...
                if msg.subscription_id == sub_ids.another_signal_signal.unwrap_or_default() {
                    ...
                    let pl: NowSignalPayload =  serde_json::from_slice(&msg.payload).expect("Failed to deserialize");
                    let _send_result = chan.send(pl);
                }
