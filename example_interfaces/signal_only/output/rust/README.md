# SignalOnly IPC Library

A Rust library providing client, server, and payload functionality for the SignalOnly IPC system: 
...
```bash
# Client example
cargo run --example signal_only_client_demo --features client

# Server example
cargo run --example signal_only_server_demo --features server

# Payloads example
cargo run --example signal_only_connection_demo --features payloads
```
